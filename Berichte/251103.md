...
| ---- | ----------------- | ------------------------------------ |
| 22   | SSH               | sicher, verschlüsselt                |
| 23   | Telnet            | unverschlüsselt, Remote-Zugriff      |
| 25   | SMTP              | E-Mail-Versand, Server-Kommunikation |
| 21   | FTP               | Dateiübertragung, Login              |
| 20   | FTP (Data)        | Datenübertragung, Port               |
...
# Nmap Optionen – Zweck & Option

- Alle Ports scannen: -p-
- Nur N häufigste Ports: --top-ports N
- Ping/Host-Discovery überspringen: -Pn
- Service/Version Detection: -sV
- Betriebssystem erkennen: -O
- Aggressiver Scan: -A
- Timing/Geschwindigkeit: -T0 bis -T5
- Ausgabe in Dateien: -oN / -oX / -oA
- Grund für Portstatus anzeigen: --reason
...
```
$ nmap  --top-ports 20 192.168.1.51
```
Starting Nmap 7.95 ( https://nmap.org ) at 2025-11-17 06:58 EST
...
912/tcp open  apex-mesh<br><br>
MAC Address: 60:A4:B7:75:7B:6A (TP-Link Limited)
...
<h1>🛠️ Nmap Cheat Sheet (Wichtig & Verwendet)</h1>

<h2>🔹 1) Grundlegende Scans</h2>
nmap &lt;IP&gt;                     # Standard-Scan (Top 1000 Ports)<br>
nmap -p- &lt;IP&gt;                 # Alle 65.535 Ports scannen<br>
nmap --top-ports &lt;N&gt; &lt;IP&gt;     # Nur die N häufigsten Ports<br>

<h2>🔹 2) Host-Discovery</h2>
nmap -sn &lt;IP&gt;                 # Host-Online-Check (Ping)<br>
nmap -Pn &lt;IP&gt;                 # Ping überspringen (wenn ICMP blockiert)<br>
nmap -n &lt;IP&gt;                  # Keine DNS-Auflösung (schneller)<br>

<h2>🔹 3) Service- & Versionsinfo</h2>
nmap -sV &lt;IP&gt;                 # Service- und Versionserkennung<br>
nmap -sC &lt;IP&gt;                 # Standard NSE-Skripte<br>

<h2>🔹 4) Tiefgehende Analyse</h2>
...
<h2>🔹 5) Geschwindigkeit / Timing</h2>
nmap -T4 &lt;IP&gt;                 # Schnell & zuverlässig (Empfohlen)<br>
nmap -T5 &lt;IP&gt;                 # Sehr schnell (wenn Genauigkeit egal ist)<br>

<h2>🔹 6) Ausgabe / Reports</h2>
...
<h2>Empfohlene Kombi-Scans für Metasploitable</h2>
nmap -sV -sC -T4 &lt;IP&gt;         # Schneller Überblick<br>
nmap -p- -T4 &lt;IP&gt;             # Vollständiger Portscan<br>
